...
- **Data plane** (`yi.shi.plinth.minio.MinioService`, `@Singleton`): wraps `io.minio.MinioClient` built from `minio.endpoint`/`minio.accessKey`/`minio.secretKey` (admin creds). `yi.shi.plinth.file.FileApi` (`@HttpService`, `@AUTH`) exposes `/file/list|search|upload|download|raw|delete|mkdir|rename|fetch-url|buckets|storage`. `GET /file/fetch-url?url=&bucket=` (`MinioService.fetchUrlToBucket`) server-side streams an http(s) URL into the user's `download/` folder via JDK `java.net.http.HttpClient` (follows redirects; auto-creates the 0-byte `download/` placeholder once; filename priority Content-Disposition `filename*`/`filename` -> final-URL path segment -> timestamp fallback, sanitized and deduped with ` (n)`; unknown Content-Length uses 10 MB multipart puts). The response is **NDJSON progress events** (`progress`/`done`/`error`, one JSON object per line, flushed as bytes flow): the controller writes directly to `response.getOutputStream()` and returns null - this works because `RestApiServiceImpl` skips `HttpRespHelper` once `response.isCommitted()` (a void/null return NPEs otherwise); failures before the first flush take the normal framework 500 JSON path. Only the scheme is restricted (http/https) - hosts are not allow-listed, so authenticated users can reach internal URLs. Rename has no native S3 primitive — `MinioService.renameObject` does a server-side `copyObject` + `removeObject` (recursively for folder prefixes, including the 0-byte placeholder). `FileApi.rename` then migrates `sys_share` rows so existing links keep working: exact `object_name` + `filename` update for files, SQL prefix replacement (`LIKE … ESCAPE '/'`) for folders. Normal users are pinned to their own bucket; admins may pass `bucket=` to view others. `DispatcherServlet` carries `@MultipartConfig` so `request.getPart("file")` works for uploads; `@HttpParam` still binds form fields. `BINARY.rawContentType` (consumed in `HttpRespHelper`) carries arbitrary content-types for downloads.
