...
public class RestApiServiceImpl implements RestApiService {
	...
	private void checkAuthAndRole(Method method) throws IOException {
		...
		if (auth.orRole().length > 0) {
			try {
				StpUtil.checkRoleOr(auth.orRole());
			}catch (Exception e){
	...
	private void invoke(Class<?> clazz, Method method, Object... args) throws Exception {
		if (Objects.nonNull(clazz)) {
			if (Objects.nonNull(method)) {
				...
				Object result = ReflectionUtils.invokeMethod(obj, method, args);
				if (!ServletHelper.getResponse().isCommitted()) {
					HttpRespHelper.sendResponseData(result, HttpStatusCode.SC_OK);
				}
			}
		}
	}
